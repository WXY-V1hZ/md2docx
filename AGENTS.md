...
`config.json` 的 `imageSize` 控制 DOCX 图片最大物理尺寸：

- `enabled`：是否启用限制。
- `maxWidthCm`：最大宽度，必须是大于 0 的有限数字，单位厘米。
- `maxHeightCm`：最大高度，必须是大于 0 的有限数字，单位厘米。

尺寸限制属于转换行为，不属于 Word 样式，禁止放入 `style-config.json`。实现位于独立的 `config/lua/limit-image-size.lua`，通过 Pandoc metadata 接收配置。filter 使用 `pandoc.mediabag.fetch()` 获取图片内容，使用 `pandoc.image.size()` 读取像素尺寸和 DPI，并按 `min(1, 最大宽度/自然宽度, 最大高度/自然高度)` 等比缩小。

- 不得放大小图。
- Markdown 已显式设置 `width` 或 `height` 时视为用户覆盖，不应用全局限制。
- 图片读取或测量失败时只警告一次并保留原图，不得中断整个转换。
- 相同 `src` 的尺寸在单次转换中缓存，避免重复读取。
- `pandoc.image.size()` 需要 Pandoc 3.1.13+；旧版本应警告并跳过限制。
- `format` 不应用 DOCX 图片尺寸，因为它不调用 Pandoc，也不应向格式化 Markdown 注入 DOCX 专属尺寸。
...
pandoc <formatted.md>
  ...
  --lua-filter=<add-inline-code.lua>
  --metadata=md2docx-image-max-width-cm:<width>
  --metadata=md2docx-image-max-height-cm:<height>
...
- `imageSize.enabled` 开启时才传入图片尺寸 metadata 和 filter；关闭时不得物化或调用该 filter。
...
- `test/image-size.test.ts`：图片自然尺寸、等比缩放、显式尺寸优先级和 DOCX DrawingML 尺寸。
...
- 图片宽高限制、等比缩小、小图不放大、显式尺寸优先和单图失败容错
