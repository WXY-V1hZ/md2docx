...
bun run index.ts   # 运行主程序（处理 test.md 输出 test_formatted.md）
bun add <pkg>      # 添加依赖
```

## 项目结构

- `index.ts` — 入口文件。读取 `test.md`，用 unified/remark 解析为 Markdown AST，调用预处理函数，将处理后的 AST 输出到 `test_formatted.md`。
- `utils/preprocess.ts` — 核心预处理逻辑，导出三个函数：
...
```
test.md → unified(remark-parse) → AST → addTitle()
                                      → normalizeHeadings()
                                      → numberHeadings()
                                    → unified(remark-stringify) → test_formatted.md
```
...
**开发守则：** 新增或修改功能后，必须先编写或更新对应的测试用例，并运行 `bun test` 确保全部通过。
